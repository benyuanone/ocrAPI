...
/*
*<p>方法 ErpTankListBandbox : <p>
*<p>说明:罐号下拉多选列表</p>
*<pre>
*@author zhou_xtian
*@date 2017-07-26 9:13
</pre>
*/
public class ErpTankListBandbox extends BaseBandbox {
...
//            }
    ...
    public void initHead() {
        ...
        if (null == head)
            head = new Listhead();
        head.setParent(lb);
        Listheader h1 = new Listheader(I18nUtil.getLabelContent(ERPTipConstants.ERP_DIC_TANK_LIST_NAME));
        h1.setParent(head);
    }
    ...
    public void loadData() {
        lb.getItems().clear();
        List<Map<String, Object>> datas = DicUtil.listDicMain("ertTankApi/listTankList", "");
        displayList(datas);
    }
    ...
    private void displayList(List<Map<String, Object>> datas) {
        ...
        for (Map<String, Object> cst : datas) {
            ...
            cell.setParent(item);
            cell.setLabel(I18nUtil.getLabelContent(cst.get("tankId").toString()));
            //判断是否选中
            if (null != getSelVals() && getSelVals().contains(cst.get("tankId").toString())) {
                item.setSelected(true);
                selLabel += cell.getLabel();
                if (!TextUtils.isEmpty(cst.get("tankId")) && cst.get("tankId").toString().equals("1")) {
    ...
    public List<Map<String,Object>> getSelVals() {
        ...
        if (null != set && set.size() > 0) {
            for (Listitem listitem : set) {
                Map<String, Object> data = (Map<String, Object>) listitem.getValue();
                if (!selVals.contains(data.get("tankId").toString()))
                    selVals.add(data);
            }
        }
        return selVals;
