...
public class BeanUtil {
    ...
    /**
     * 返回对象指定属性Getter方法的Hibernate Annotation中定义的字段长度<br/>
     * 具体来说，就是@Column的length属性。
     *
     * @param clazz     类
     * @param fieldName 属性名
     * @return 若出错返回null
     */
    public static Integer getAnnotationLength(Class<?> clazz, String fieldName) {
        //另需要考虑，如果clazz是Hibernate代理类，需要获得真实的类
        try {
            Method m = PropertyUtils.getReadMethod(new PropertyDescriptor(
                    fieldName, getRealClassFromHibernate(clazz)));
            // 获得Hibernate的Column Annotation
            Annotation annotation = AnnotationUtils.getAnnotation(m,
                    Column.class);
            if (annotation != null) {
                return (Integer) AnnotationUtils.getValue(annotation,
                        COLUMN_LENGTH); // 获取length属性
            } else {
                return null;
            }
        } catch (Exception e) {
//            log.error(e.getMessage(), e);
    ...
    /**
     * 返回对象指定属性Getter方法的Hibernate Annotation中定义的字段nullable<br/>
     *
     * @param clazz     类
     * @param fieldName 属性名
     * @return 存在返回具体设定的值，否则返回false
     */
    public static Boolean getAnnotationNullable(Class<?> clazz, String fieldName) {
        // 另需要考虑，如果clazz是Hibernate代理类，需要获得真实的类
        try {
            Method m = PropertyUtils.getReadMethod(new PropertyDescriptor(fieldName, getRealClassFromHibernate(clazz)));
            // 获得Hibernate的Column Annotation
            Annotation annotation = AnnotationUtils.getAnnotation(m, Column.class);
            if (annotation != null) {
                Boolean result = (Boolean) AnnotationUtils.getValue(annotation, COLUMN_NULLABLE); // 获取NULLABLE属性
...
//        }
    ...
    /**
     * 复制srcEntity对象中的属性到destEntity对象同名同数据类型的属性中
     * 提示：如果filterType方式为CommonConstant.FILTER_TYPE_INCLUDE则只复制的属性为filterProperty
     * 如果filterType方式为CommonConstant.FILTER_TYPE_EXCLUDE则只复制filterProperty以外的属性
     * 本方法目前只对基本的java数据类型的属性进行浅copy
     * 支持多层（嵌套）属性值复制
     *
     * @param srcEntity      源对象
     * @param destEntity     目标对象
     * @param filterProperty 复制属性名称（复制策略根据filterType）注意：如果有复制成功则会删除对应的属性名称，到最后只剩没有变动属性名称
     * @param filterType     CommonConstant.FILTER_TYPE_INCLUDE则只复制的属性为filterProperty;CommonConstant.FILTER_TYPE_EXCLUDE则只复制filterProperty以外的属性
     */
    public static void copyBean(Object srcEntity, Object destEntity, Set<String> filterProperty, String filterType) {
        if (destEntity == null || srcEntity == null) {
            return;
        }
        if (srcEntity instanceof HibernateProxy) {
            HibernateProxy proxy = (HibernateProxy) srcEntity;
            srcEntity = proxy.getHibernateLazyInitializer().getImplementation();
        }
        PropertyDescriptor[] pds = getPropertyDescriptors(srcEntity.getClass());
        for (PropertyDescriptor srcPd : pds) {
            String property = srcPd.getName();
            if (!property.equals("class"))//过虑class
            {
                /*
                 * 如果有指定不包含需要update的property则过虑
				 */
                if (filterProperty != null) {
                    if (CommonConstants.FILTER_TYPE_EXCLUDE.equalsIgnoreCase(filterType)) {
                        if (filterProperty.contains(property)) {
                            continue;
                        }
                    } else {
                        if (!filterProperty.contains(property)) {
                ...
                try {
                     ...
                     * 如果有相同的属性名称
					 */
                    PropertyDescriptor targetPd = getPropertyDescriptor(destEntity, property);

					/*
                     * 不是所属注册的数据类型不做处理
					 */
                    if (targetPd == null || !CompareUtil.isRegDataType(targetPd.getPropertyType())) {
                        continue;
                    }
                    /*
                     * 如果数据类型相同且值不相同则复制updatedBean到oldBean对应的值
					 */
                    if (srcPd.getPropertyType().equals(targetPd.getPropertyType()) ||
                            isCompatibleType(srcPd.getPropertyType(), targetPd.getPropertyType())) {
                        Object targetValue = PropertyUtils.getProperty(destEntity, property);
                        Object srcValue = PropertyUtils.getProperty(srcEntity, property);
                        /*
                         * 处理符合一般是java数据类型
						 */
                        if (CompareUtil.isNotEqual(targetValue, srcValue)) {
                            setProperty(destEntity, property, srcValue);
                            if (filterProperty != null) {
                                filterProperty.remove(property);
                            }
                        }
                    }
                } catch (Exception e) {
