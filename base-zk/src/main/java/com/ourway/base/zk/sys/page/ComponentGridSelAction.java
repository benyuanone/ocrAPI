 ...
 * </pre>
 */
public class ComponentGridSelAction extends BaseWindow {
    protected Log info = LogFactory.getLog(ComponentGridSelAction.class);
    ...
    public void onCreate(CreateEvent event) {
        ...
        BaseButton removeBtn = (BaseButton) getFellowIfAny("button_removeBtn");
    ...
    //删除当前表格
    void removeGrid() {
        Tabpanel tabpanel = ZkUtils.getParentPanel(this.getParent());
        String tabPanelId = tabpanel.getId();
        String tabId = tabPanelId.replaceAll("_panel", "");
        BaseTabbox _tabbox = (BaseTabbox) tabpanel.getParent().getParent();
        Tabs _tabs = _tabbox.getTabs();
        Tab tab = (Tab) _tabs.getFellowIfAny(tabId);
        tabpanel.getParent().removeChild(tabpanel);
        _tabs.removeChild(tab);
        PageComponentAction pa = (PageComponentAction)_tabs.getParent().getParent();
        pa.winComponents.remove(tabId);
    }
...
//        Panelchildren btnGroup = (Panelchildren) getFellowIfAny("button_group1" + data.get("compHbox").toString());
//        if (null == btnGroup) {
...
//            dragBtn.setExternObject(sourBtn);
    ...
    public void onEvent(Event event) throws Exception {
        ...
        if (comp instanceof BaseButton) {
            ...
            if (comp.getId().equals("button_removeBtn")) {
                removeGrid();
            }
        }
    }
