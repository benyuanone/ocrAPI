 ...
 * </pre>
 */
public class ComponentButtonSelAction extends BaseWindow {
    protected Log info = LogFactory.getLog(ComponentButtonSelAction.class);
        ...
        if (null != ppt.get("dataList")) {
            ...
            for (Map<String, Object> data : datas) {
                ...
                if (TextUtils.isEmpty(data.get("compHbox")))
                    pc = doCreateChildren(1);
                else
                    pc = doCreateChildren((Integer) data.get("compHbox"));
                button.setParent(pc);
    ...
    void addButton2Grid() {
        ...
        if (null != datas && datas.size() > 0) {
            for (Map<String, Object> data : datas) {
                ...
                if(!TextUtils.isEmpty(OSCacheFactory.getInstance().get(data.get("kjAttribute").toString())))
                    _data.put("columnDisable",OSCacheFactory.getInstance().get(data.get("kjAttribute").toString()));
