...
//        this.addEventListener(Events.ON_BLUR, new EventHandle());
...
//                Map<String, Object> obj = window.filterByKey(getText());
//                if (null != obj) {
    ...
    public void setSelVals(List<Map<String, Object>> selVals) {
        if (null != selVals && selVals.size() >= 1) {
            ...
            if (null != map.get("bean")) {
                MenuVO menuVO = (MenuVO) map.get("bean");
                Map<String, Object> _menuMap = JsonUtil.jsonToMap(JsonUtil.toJson(menuVO));
                map.put("bean", _menuMap);
            }
            if (!TextUtils.isEmpty(bandboxVo.getControlClass())) {
