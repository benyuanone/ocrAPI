 ...
 * </ul>
 */
public class TreeLoadAction implements TreeListinerSer {
    ...
    public void doAction(BaseWindow window, Event event, BaseTree tree, Treeitem treeitem, PageControlVO pgvo) {
        ...
        List<TreeVO> datas = TreeDataUtil.loadTree(_params.get("url").toString());
        if (null != datas && datas.size() > 0)
            TreeUtils.showTree(window, treeitem, tree, datas, selItem);
    }
