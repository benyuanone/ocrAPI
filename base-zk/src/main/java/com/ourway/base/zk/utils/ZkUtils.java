...
/**
 * 24. * ZkUtils集合了zk中常用的一些功能，方便开发中的使用 25. * 26. * 27. *
 * 
 * @author Jack *
 */
...
public class ZkUtils {
	...
	//处理打开窗口的级次问题
	public static List<String> doHandleTabid(BaseWindow window, String tabId){
		//创建窗体中的级联窗体
//		List<String> tabs = null;
//		if(null!=window.getTabId())
//			tabs = window.getTabId();
//		else
//			tabs = new ArrayList<String>();
	//	List<String> _tabs = window.getTabId();
	//	if(null!=_tabs&&_tabs.size()>0){
...
//		return tabs;
	...
	public static BaseGrid getBaseGrid(Component component){
		if(component instanceof BaseGrid)
			return (BaseGrid)component;
		else
			return getBaseGrid(component.getParent());
	}
	//获取最近的tabpanel
	public static Tabpanel getParentPanel(Component component){
		if(component instanceof Tabpanel)
			return (Tabpanel)component;
		else
			return getParentPanel(component.getParent());
	}
	 ...
	 * dropped. Parameters: uri the URI to redirect to, or null to reload the
	 * same page
	 */
	 ...
	 * href="http://sunflowers.javaeye.com/blog/686243">这里<a>
	 * 
	 * @param message
	 * @param title
	 *            窗口标题
	...
	/**
	 * 错误提示框 <b style="color:red;"/>注意</b>：自从zk5.0以后，默认禁用本地事件进程,见zk.xml配置文件
	 * &lt;disable-event-thread&gt;true&lt;/disable-event-thread&gt;启用事件处理线程，
	 * 请将true改为false
	 * ,如果此行注释掉或者没有此行，请添加配置&lt;disable-event-thread&gt;false&lt;/disable
	 * &gt;当禁用事件线程时，如果您使用了messagebox的返回值作为判断的话，那么if语句内的代码永远都不会执行。这两个的区别的详细说明见 <a
	...
	/**
	 * 获得指定组件的根节点 ，注意，可能返回其自身，当其是根节点或者为null的时候，
	 * 
	 * @since 3.6.3
	 */
	...
	public static boolean isAncestor(Component node1, Component node2) {
		return Components.isAncestor(node1, node2);// 该方法没有null判断
	}
