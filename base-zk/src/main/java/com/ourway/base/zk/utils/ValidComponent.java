...
public class ValidComponent {
    ...
    //通用校验类
    public static boolean valid(String constraint, Object val, BaseWindow window) {
        if (TextUtils.isEmpty(constraint))
            return true;
        String[] str = constraint.split("\\|");
        boolean flag = true;
        for (String s : str) {
            if (!TextUtils.isEmpty(s)) {
                if (s.equalsIgnoreCase("no empty")) {
                    if (!checkNoEmpty(val))
                        return false;
                }
    ...
    //非空
    public static boolean checkNoEmpty(Object val) {
     ...
     * 验证身份证号码 15位 18位
     *
     * @param cardStrObj 身份证字符串
     * @return true 合法 false 不合法
     */
    public static boolean isValidIdCard(Object cardStrObj) {
        if (TextUtils.isEmpty(cardStrObj))
            return true;
        String cardStr = cardStrObj.toString();
        boolean flag = false;
     ...
     * 验证手机号码
     *
     * @param mobileNo 电话号码字符串 130到139 和 150，152 ，157，158，159 ，186，189，187
     * @return
     */
